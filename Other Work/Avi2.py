...
class robot:
    ...
    def calcRectanglePoints(self):
        ...
        angle = 90
        angle2 = 90
        diff = m.dist([self.robotPerimeter[0], 0], [self.robotPerimeter[2], 0]) / m.dist([self.robotPerimeter[0],
                                                                                          self.robotPerimeter[1]],
                                                                                         [self.robotPerimeter[2],
                                                                                          self.robotPerimeter[3]])
        print("var diff in radian")
        print(diff)
        if (self.robotPerimeter[1] >= self.robotPerimeter[3]): # left wheel is higher on y axis
            angle = 180 - 90 - (m.acos(diff)*180/m.pi)
        else:# right wheel is higher on y axis
            angle = (m.acos(diff)*180/m.pi) + 90
        print("angle between the orientation of the car and y axis")
        print(angle)

        self.robotPerimeter[0] = self.robotPerimeter[0] - m.cos(angle) * self.w1velocity
        self.robotPerimeter[1] = self.robotPerimeter[1] - m.sin(angle) * self.w1velocity

        self.robotPerimeter[2] = self.robotPerimeter[2] - m.cos(angle) * self.w2velocity
        self.robotPerimeter[3] = self.robotPerimeter[3] - m.sin(angle) * self.w2velocity

        if (self.robotPerimeter[1] >= self.robotPerimeter[3]):
            angle2 = 180 - 90 - (m.acos(diff)*180/m.pi)
        else:
            angle2 = (m.acos(diff)*180/m.pi) + 90
    ...
    def drawRobotMove(self, canv):
        self.drawRobot = canv.create_rectangle(self.robotPerimeter[0], self.robotPerimeter[1],
                                              self.robotPerimeter[6], self.robotPerimeter[7], outline='#000000')
...
button2 = tk.Button(
    window,
    text="Add Robot",
    command=lambda: env.addRobot(int(text_box4.get(1.0, 'end')), int(text_box5.get(1.0, 'end')), text_box6.get(1.0, 'end'))
)
