...
class robot:
    ...
    def calcWheelSpeeds(self, lightList):
        ...
        self.w1velocity = abs(int(self.k[0]) * s1 + int(self.k[1]) * s2)
        self.w2velocity = abs(int(self.k[2]) * s1 + int(self.k[3]) * s2)
    ...
    def calcRectanglePoints(self, lightList):
        ...
        if self.w2velocity == self.w1velocity:
            ...
            if self.w1velocity > self.w2velocity:
                ...
                omega = (self.w1velocity - self.w2velocity) / (2*self.width)
                ...
                center = rotatePoint(icc, center, 10*omega)
